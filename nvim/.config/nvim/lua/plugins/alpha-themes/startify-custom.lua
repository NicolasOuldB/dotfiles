...
local max_width = 0
for _, line in ipairs(default_header.val) do
...
local function truncate_middle_exact(s, max_len)
    ...
    local front_len = math.floor(keep_len / 2)
    local back_len = keep_len - front_len

    local front = s:sub(1, front_len)
    local back = s:sub(-back_len)
...
local function mru(start, cwd, items_number, opts)
    ...
    for i, fn in ipairs(oldfiles) do
        ...
        short_fn = truncate_middle_exact(short_fn, max_width)
